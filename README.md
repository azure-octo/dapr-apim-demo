...
<policies>
     <inbound>
          <base />
          <publish-to-dapr topic="@("demo-events/messages")" template="liquid">{{body}}</publish-to-dapr>
     </inbound>
     ...
</policies>
```
...
If you are making changes to the component you will have to restart the `event-subscriber` deployment 
